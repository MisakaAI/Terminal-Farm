# main.py
import time
from blessed import Terminal
from core.save import load_save, save_game
from core.player import Player
from core.world import World
from renderer.ascii_renderer import AsciiRenderer

VIEW_W, VIEW_H = 50, 20

def main():
    term = Terminal()
    save_data = load_save()

    # 加载地图
    map_path = save_data.get("world", "map/pasture.npy")
    world = World(map_path)

    # 从存档中加载玩家位置
    player_pos = save_data.get("player", {"x": 10, "y": 10})
    player = Player(
        x=player_pos.get("x", 10),
        y=player_pos.get("y", 10)
    )

    renderer = AsciiRenderer(term=term, view_w=VIEW_W, view_h=VIEW_H)

    with term.fullscreen(), term.cbreak(), term.hidden_cursor():
        key = ""
        debug_msgs = ["Welcome to the world!"]
        last_tick = time.time()

        while key.lower() != "q":
            now = time.time()

            # 每秒刷新（模拟游戏时间）
            if now - last_tick >= 1.0:
                last_tick = now

            # 绘制地图与玩家
            renderer.draw(world, player, debug_lines=debug_msgs)

            # 读取按键（非阻塞）
            key = term.inkey(timeout=0.05)
            moved = False

            if key:
                # 处理方向键
                if key.name == "KEY_UP":
                    moved = player.move(0, -1, world)
                elif key.name == "KEY_DOWN":
                    moved = player.move(0, 1, world)
                elif key.name == "KEY_LEFT":
                    moved = player.move(-1, 0, world)
                elif key.name == "KEY_RIGHT":
                    moved = player.move(1, 0, world)

                elif key.lower() == "s":
                    # 手动保存
                    save_data["player"] = {"x": player.x, "y": player.y}
                    save_game(save_data)
                    debug_msgs = [f"Saved at ({player.x},{player.y})"]

                elif key.lower() == "q":
                    break

if __name__ == "__main__":
    main()
